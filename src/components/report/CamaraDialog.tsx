...
export function CamaraDialog({
  ...
  const [facingMode, setFacingMode] = useState<"user" | "environment">("user"); // 👈 modo actual
  const videoRef = useRef<HTMLVideoElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const streamRef = useRef<MediaStream | null>(null);

  // 🔄 Iniciar cámara con el modo actual
  const startCamera = async () => {
    try {
      setIsLoading(true);
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode }, // 👈 usamos el estado
      });
  ...
  // ▶️ Manejar apertura/cierre del diálogo
  useEffect(() => {
    if (open) {
      startCamera();
    } else {
  ...
  }, [open, facingMode]); // 👈 reiniciar cámara al cambiar facingMode

  const capturePhoto = () => {
    if (!videoRef.current || !canvasRef.current || files.length >= MAX_FILES)
      return;
  ...
  const currentPhotosCount = files.filter((f) =>
        ...
        <DialogHeader className="p-4 border-b">
          ...
          <DialogDescription>
            ...
            {currentPhotosCount > 0 &&
              ` (${currentPhotosCount}/${MAX_FILES} tomadas)`}
          </DialogDescription>
        </DialogHeader>
